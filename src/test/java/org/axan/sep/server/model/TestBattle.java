...
public class TestBattle
{
	...
	{	
		...
		for(StarshipTemplate championSizeTemplate : SEPUtils.starshipSizeTemplates)
		{
			...
			for(StarshipTemplate chalengerSizeTemplate : SEPUtils.starshipSizeTemplates)
			{
				...
				out.print("\n\"Ego VS TdT\"\t\""+doBattle(scale, champion, chalengerSizeTemplate, championClass.getTdT())+"\"");
				
				out.print("\n\"Ego VS Ego\"\t\""+doBattle(scale, champion, chalengerSizeTemplate, championClass)+"\"");				
				
				out.print("\n\"Ego VS BN\"\t\""+doBattle(scale, champion, chalengerSizeTemplate, championClass.getBN())+"\"");
	...
	private String doBattle(int scale, StarshipTemplate champion, StarshipTemplate chalengerSizeTemplate, eStarshipSpecializationClass chalengerClass)
	{
		...
		Stack<Integer> drawCuts = new Stack<Integer>(); drawCuts.push(maxUnits);
		Stack<Integer> looseCuts = new Stack<Integer>(); looseCuts.push(maxUnits);
		...
		if (!championIsWeak)
		{
			return "Win from 0 to "+winCut+" chalenger(s), "+(drawCuts.peek() < looseCuts.peek() ? "Draw from "+(winCut+1)+" to "+(looseCuts.peek()-1)+" chalenger(s), ":"")+"Loose from "+looseCuts.peek()+" chalenger(s).";
		}
		else
		{
			return "\"\t\"Loose from 0 to "+winCut+" champion(s), "+(drawCuts.peek() < looseCuts.peek() ? "Draw from "+(winCut+1)+" to "+(looseCuts.peek()-1)+", champion(s), ":"")+"Win from "+looseCuts.peek()+" champion(s).";			
		}
	}
}
