...
public class TristateCheckBox extends JCheckBox
{
	...
	public static final State		PARTIALY_SELECTED = new State();
	...
	/**
	 * Exactly which Design Pattern is this? Is it an Adapter, a Proxy or a Decorator? In this case, my vote lies with the Decorator, because we are extending functionality and "decorating" the original model with a more powerful model.
	 */
	private class TristateDecorator implements ButtonModel
	{
		...
		private void setState(State state)
		{
			...
			else if (state == PARTIALY_SELECTED)
			{	// 1 1 0
				other.setArmed(true);
				setPressed(true);
				setSelected(false);
		...
		{
			if (!isArmed() && !isPressed() && isSelected())
			{
				return SELECTED;
			}
			else if (!isArmed() && !isPressed() && !isSelected())
			{
				return NOT_SELECTED;
			}
			else if (isArmed() && isPressed() && !isSelected())
			{
				return PARTIALY_SELECTED;
			}
			else
			{
				return DONT_CARE;
		...
		/** We rotate between NOT_SELECTED, SELECTED, PARTIALY_SELECTED and DONT_CARE. */
		private void nextState()
		{
			...
			{
				setState(DONT_CARE); //setState(PARTIALY_SELECTED);
			}
			else if (current == PARTIALY_SELECTED)
			{
				setState(DONT_CARE);
			}
