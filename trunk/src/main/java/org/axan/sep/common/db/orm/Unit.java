...
public class Unit implements IUnit
{
	...
	Unit(IBaseUnit baseUnitProxy, IGameConfig config)
	{
		...
		try
		{
			ty = eUnitType.valueOf(baseUnitProxy.getType());
		}
		catch(Throwable t)
		{
			ty = eUnitType.valueOf(baseUnitProxy.getType());
	...
	public Unit(String owner, String name, eUnitType type, IGameConfig config)
	{
		this(new BaseUnit(owner, name, type.toString()), config);
	}
	...
	public static <T extends IUnit> Set<T> selectMaxVersion(SEPCommonDB db, Class<T> expectedType, Integer maxVersion, String from, String where, Object ... params) throws SQLDataBaseException
	{
		return select(db, expectedType, true, maxVersion, from, where, params);
	}

	public static <T extends IUnit> Set<T> selectVersion(SEPCommonDB db, Class<T> expectedType, int version, String from, String where, Object ... params) throws SQLDataBaseException
	{
		return select(db, expectedType, false, version, from, where, params);
	}

	public static <T extends IUnit> Set<T> selectUnversioned(SEPCommonDB db, Class<T> expectedType, String from, String where, Object ... params) throws SQLDataBaseException
	{
		return select(db, expectedType, false, null, from, where, params);
	}

	private static <T extends IUnit> Set<T> select(SEPCommonDB db, Class<T> expectedType, boolean maxVersion, Integer version, String from, String where, Object ... params) throws SQLDataBaseException
	{
		...
		try
		{
			...
			while(stmnt.step())
			{
				...
				IUnit o = DataBaseORMGenerator.mapTo(clazz, stmnt, db.getConfig());
	...
	/** Set maxVersion to null to select last version. */
	public static <T extends IUnit> boolean existMaxVersion(SEPCommonDB db, Class<T> expectedType, Integer maxVersion, String from, String where, Object ... params) throws SQLDataBaseException
	{
		return exist(db, expectedType, true, maxVersion, from, where, params);
	}

	public static <T extends IUnit> boolean existVersion(SEPCommonDB db,Class<T> expectedType, int version, String from, String where, Object ... params) throws SQLDataBaseException
	{
		return exist(db, expectedType, false, version, from, where, params);
	}

	public static <T extends IUnit> boolean existUnversioned(SEPCommonDB db, Class<T> expectedType, String from, String where, Object ... params) throws SQLDataBaseException
	{
		return exist(db, expectedType, false, null, from, where, params);
	}

	private static <T extends IUnit> boolean exist(SEPCommonDB db, Class<T> expectedType, boolean maxVersion, Integer version, String from, String where, Object ... params) throws SQLDataBaseException
	{
		try
		{
	...
	public static <T extends IUnit> void insertOrUpdate(SEPCommonDB db, T unit) throws SQLDataBaseException
	{
		try
		{
			boolean exist = existUnversioned(db, unit.getClass(), null, " Unit.owner = %s AND Unit.name = %s", "'"+unit.getOwner()+"'", "'"+unit.getName()+"'");
			IVersionedUnit vunit = (IVersionedUnit.class.isInstance(unit) ? IVersionedUnit.class.cast(unit) : null);
			boolean vexist = existVersion(db, vunit.getClass(), vunit.getTurn(), null, " VersionedUnit.owner = %s AND VersionedUnit.name = %s AND VersionedUnit.turn = %s", "'"+vunit.getOwner()+"'", "'"+vunit.getName()+"'", "'"+vunit.getTurn()+"'");
			if (vexist && !exist) throw new Error("Versioned Unit cannot exist without unversioned entry.");
			if (exist)
			{
				db.getDB().exec(String.format("UPDATE Unit SET type = %s WHERE  Unit.owner = %s AND Unit.name = %s ;", "'"+unit.getType()+"'", "'"+unit.getOwner()+"'", "'"+unit.getName()+"'").replaceAll("'null'", "NULL"));
				if (vexist && vunit != null)
				{
					db.getDB().exec(String.format("UPDATE VersionedUnit SET type = %s,  departure_x = %s,  departure_y = %s,  departure_z = %s,  progress = %s,  destination_x = %s,  destination_y = %s,  destination_z = %s WHERE  VersionedUnit.owner = %s AND VersionedUnit.name = %s AND VersionedUnit.turn = %s ;", "'"+vunit.getType()+"'", vunit.getDeparture() == null ? "NULL" : "'"+vunit.getDeparture().x+"'", vunit.getDeparture() == null ? "NULL" : "'"+vunit.getDeparture().y+"'", vunit.getDeparture() == null ? "NULL" : "'"+vunit.getDeparture().z+"'", "'"+vunit.getProgress()+"'", vunit.getDestination() == null ? "NULL" : "'"+vunit.getDestination().x+"'", vunit.getDestination() == null ? "NULL" : "'"+vunit.getDestination().y+"'", vunit.getDestination() == null ? "NULL" : "'"+vunit.getDestination().z+"'", "'"+vunit.getOwner()+"'", "'"+vunit.getName()+"'", "'"+vunit.getTurn()+"'").replaceAll("'null'", "NULL"));
				}
				switch(unit.getType())
				{
					case PulsarMissile:
					{
						...
						db.getDB().exec(String.format("UPDATE PulsarMissile SET type = %s,  time = %s,  volume = %s WHERE  PulsarMissile.owner = %s AND PulsarMissile.name = %s ;", "'"+pulsarMissile.getType()+"'", "'"+pulsarMissile.getTime()+"'", "'"+pulsarMissile.getVolume()+"'", "'"+pulsarMissile.getOwner()+"'", "'"+pulsarMissile.getName()+"'").replaceAll("'null'", "NULL"));
						if (vexist && vunit != null)
						{
							IVersionedPulsarMissile versionedPulsarMissile = IVersionedPulsarMissile.class.cast(unit);
							db.getDB().exec(String.format("UPDATE VersionedPulsarMissile SET type = %s,  direction_x = %s,  direction_y = %s,  direction_z = %s WHERE  VersionedPulsarMissile.owner = %s AND VersionedPulsarMissile.name = %s AND VersionedPulsarMissile.turn = %s ;", "'"+versionedPulsarMissile.getType()+"'", versionedPulsarMissile.getDirection() == null ? "NULL" : "'"+versionedPulsarMissile.getDirection().x+"'", versionedPulsarMissile.getDirection() == null ? "NULL" : "'"+versionedPulsarMissile.getDirection().y+"'", versionedPulsarMissile.getDirection() == null ? "NULL" : "'"+versionedPulsarMissile.getDirection().z+"'", "'"+versionedPulsarMissile.getOwner()+"'", "'"+versionedPulsarMissile.getName()+"'", "'"+versionedPulsarMissile.getTurn()+"'").replaceAll("'null'", "NULL"));
						}
						break;
					}
					case Probe:
					{
						...
						if (vexist && vunit != null)
						{
							...
							db.getDB().exec(String.format("UPDATE VersionedProbe SET type = %s WHERE  VersionedProbe.owner = %s AND VersionedProbe.name = %s AND VersionedProbe.turn = %s ;", "'"+versionedProbe.getType()+"'", "'"+versionedProbe.getOwner()+"'", "'"+versionedProbe.getName()+"'", "'"+versionedProbe.getTurn()+"'").replaceAll("'null'", "NULL"));
						}
						break;
					}
					case AntiProbeMissile:
					{
						...
						db.getDB().exec(String.format("UPDATE AntiProbeMissile SET type = %s WHERE  AntiProbeMissile.owner = %s AND AntiProbeMissile.name = %s ;", "'"+antiProbeMissile.getType()+"'", "'"+antiProbeMissile.getOwner()+"'", "'"+antiProbeMissile.getName()+"'").replaceAll("'null'", "NULL"));
						if (vexist && vunit != null)
						{
							IVersionedAntiProbeMissile versionedAntiProbeMissile = IVersionedAntiProbeMissile.class.cast(unit);
							db.getDB().exec(String.format("UPDATE VersionedAntiProbeMissile SET type = %s,  targetOwner = %s,  targetName = %s,  targetTurn = %s WHERE  VersionedAntiProbeMissile.owner = %s AND VersionedAntiProbeMissile.name = %s AND VersionedAntiProbeMissile.turn = %s ;", "'"+versionedAntiProbeMissile.getType()+"'", "'"+versionedAntiProbeMissile.getTargetOwner()+"'", "'"+versionedAntiProbeMissile.getTargetName()+"'", "'"+versionedAntiProbeMissile.getTargetTurn()+"'", "'"+versionedAntiProbeMissile.getOwner()+"'", "'"+versionedAntiProbeMissile.getName()+"'", "'"+versionedAntiProbeMissile.getTurn()+"'").replaceAll("'null'", "NULL"));
						}
						break;
					}
					case Fleet:
					{
						...
						if (vexist && vunit != null)
						{
							...
							db.getDB().exec(String.format("UPDATE VersionedFleet SET type = %s WHERE  VersionedFleet.owner = %s AND VersionedFleet.name = %s AND VersionedFleet.turn = %s ;", "'"+versionedFleet.getType()+"'", "'"+versionedFleet.getOwner()+"'", "'"+versionedFleet.getName()+"'", "'"+versionedFleet.getTurn()+"'").replaceAll("'null'", "NULL"));
						}
						break;
					}
					case CarbonCarrier:
					{
						...
						db.getDB().exec(String.format("UPDATE CarbonCarrier SET type = %s,  sourceType = %s,  sourceCelestialBodyName = %s,  sourceTurn = %s WHERE  CarbonCarrier.owner = %s AND CarbonCarrier.name = %s ;", "'"+carbonCarrier.getType()+"'", "'"+carbonCarrier.getSourceType()+"'", "'"+carbonCarrier.getSourceCelestialBodyName()+"'", "'"+carbonCarrier.getSourceTurn()+"'", "'"+carbonCarrier.getOwner()+"'", "'"+carbonCarrier.getName()+"'").replaceAll("'null'", "NULL"));
						if (vexist && vunit != null)
						{
							IVersionedCarbonCarrier versionedCarbonCarrier = IVersionedCarbonCarrier.class.cast(unit);
							db.getDB().exec(String.format("UPDATE VersionedCarbonCarrier SET type = %s,  orderOwner = %s,  orderSource = %s,  orderPriority = %s WHERE  VersionedCarbonCarrier.owner = %s AND VersionedCarbonCarrier.name = %s AND VersionedCarbonCarrier.turn = %s ;", "'"+versionedCarbonCarrier.getType()+"'", "'"+versionedCarbonCarrier.getOrderOwner()+"'", "'"+versionedCarbonCarrier.getOrderSource()+"'", "'"+versionedCarbonCarrier.getOrderPriority()+"'", "'"+versionedCarbonCarrier.getOwner()+"'", "'"+versionedCarbonCarrier.getName()+"'", "'"+versionedCarbonCarrier.getTurn()+"'").replaceAll("'null'", "NULL"));
						}
						break;
					}
					case SpaceRoadDeliverer:
					{
						...
						if (vexist && vunit != null)
						{
							...
							db.getDB().exec(String.format("UPDATE VersionedSpaceRoadDeliverer SET type = %s WHERE  VersionedSpaceRoadDeliverer.owner = %s AND VersionedSpaceRoadDeliverer.name = %s AND VersionedSpaceRoadDeliverer.turn = %s ;", "'"+versionedSpaceRoadDeliverer.getType()+"'", "'"+versionedSpaceRoadDeliverer.getOwner()+"'", "'"+versionedSpaceRoadDeliverer.getName()+"'", "'"+versionedSpaceRoadDeliverer.getTurn()+"'").replaceAll("'null'", "NULL"));
						}
						break;
					}
				}
			...
			if (!exist || !vexist)
			{
				if (!exist) db.getDB().exec(String.format("INSERT INTO Unit (owner, name, type) VALUES (%s, %s, %s);", "'"+unit.getOwner()+"'", "'"+unit.getName()+"'", "'"+unit.getType()+"'").replaceAll("'null'", "NULL"));
				if (vunit != null)
				{
					db.getDB().exec(String.format("INSERT INTO VersionedUnit (owner, name, turn, type, departure_x, departure_y, departure_z, progress, destination_x, destination_y, destination_z) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);", "'"+vunit.getOwner()+"'", "'"+vunit.getName()+"'", "'"+vunit.getTurn()+"'", "'"+vunit.getType()+"'", vunit.getDeparture() == null ? "NULL" : "'"+vunit.getDeparture().x+"'", vunit.getDeparture() == null ? "NULL" : "'"+vunit.getDeparture().y+"'", vunit.getDeparture() == null ? "NULL" : "'"+vunit.getDeparture().z+"'", "'"+vunit.getProgress()+"'", vunit.getDestination() == null ? "NULL" : "'"+vunit.getDestination().x+"'", vunit.getDestination() == null ? "NULL" : "'"+vunit.getDestination().y+"'", vunit.getDestination() == null ? "NULL" : "'"+vunit.getDestination().z+"'").replaceAll("'null'", "NULL"));
				}
				switch(unit.getType())
				{
					case PulsarMissile:
					{
						IPulsarMissile pulsarMissile = IPulsarMissile.class.cast(unit);
						if (!exist) db.getDB().exec(String.format("INSERT INTO PulsarMissile (owner, name, type, time, volume) VALUES (%s, %s, %s, %s, %s);", "'"+pulsarMissile.getOwner()+"'", "'"+pulsarMissile.getName()+"'", "'"+pulsarMissile.getType()+"'", "'"+pulsarMissile.getTime()+"'", "'"+pulsarMissile.getVolume()+"'").replaceAll("'null'", "NULL"));
						if (vunit != null)
						{
							IVersionedPulsarMissile versionedPulsarMissile = IVersionedPulsarMissile.class.cast(unit);
							db.getDB().exec(String.format("INSERT INTO VersionedPulsarMissile (owner, name, turn, type, direction_x, direction_y, direction_z) VALUES (%s, %s, %s, %s, %s, %s, %s);", "'"+versionedPulsarMissile.getOwner()+"'", "'"+versionedPulsarMissile.getName()+"'", "'"+versionedPulsarMissile.getTurn()+"'", "'"+versionedPulsarMissile.getType()+"'", versionedPulsarMissile.getDirection() == null ? "NULL" : "'"+versionedPulsarMissile.getDirection().x+"'", versionedPulsarMissile.getDirection() == null ? "NULL" : "'"+versionedPulsarMissile.getDirection().y+"'", versionedPulsarMissile.getDirection() == null ? "NULL" : "'"+versionedPulsarMissile.getDirection().z+"'").replaceAll("'null'", "NULL"));
						}
						break;
					}
					case Probe:
					{
						...
						{
							IVersionedProbe versionedProbe = IVersionedProbe.class.cast(unit);
							db.getDB().exec(String.format("INSERT INTO VersionedProbe (owner, name, turn, type) VALUES (%s, %s, %s, %s);", "'"+versionedProbe.getOwner()+"'", "'"+versionedProbe.getName()+"'", "'"+versionedProbe.getTurn()+"'", "'"+versionedProbe.getType()+"'").replaceAll("'null'", "NULL"));
						}
						break;
					}
					case AntiProbeMissile:
					{
						IAntiProbeMissile antiProbeMissile = IAntiProbeMissile.class.cast(unit);
						if (!exist) db.getDB().exec(String.format("INSERT INTO AntiProbeMissile (owner, name, type) VALUES (%s, %s, %s);", "'"+antiProbeMissile.getOwner()+"'", "'"+antiProbeMissile.getName()+"'", "'"+antiProbeMissile.getType()+"'").replaceAll("'null'", "NULL"));
						if (vunit != null)
						{
							IVersionedAntiProbeMissile versionedAntiProbeMissile = IVersionedAntiProbeMissile.class.cast(unit);
							db.getDB().exec(String.format("INSERT INTO VersionedAntiProbeMissile (owner, name, turn, type, targetOwner, targetName, targetTurn) VALUES (%s, %s, %s, %s, %s, %s, %s);", "'"+versionedAntiProbeMissile.getOwner()+"'", "'"+versionedAntiProbeMissile.getName()+"'", "'"+versionedAntiProbeMissile.getTurn()+"'", "'"+versionedAntiProbeMissile.getType()+"'", "'"+versionedAntiProbeMissile.getTargetOwner()+"'", "'"+versionedAntiProbeMissile.getTargetName()+"'", "'"+versionedAntiProbeMissile.getTargetTurn()+"'").replaceAll("'null'", "NULL"));
						}
						break;
					}
					case Fleet:
					{
						...
						{
							IVersionedFleet versionedFleet = IVersionedFleet.class.cast(unit);
							db.getDB().exec(String.format("INSERT INTO VersionedFleet (owner, name, turn, type) VALUES (%s, %s, %s, %s);", "'"+versionedFleet.getOwner()+"'", "'"+versionedFleet.getName()+"'", "'"+versionedFleet.getTurn()+"'", "'"+versionedFleet.getType()+"'").replaceAll("'null'", "NULL"));
						}
						break;
					}
					case CarbonCarrier:
					{
						...
						{
							IVersionedCarbonCarrier versionedCarbonCarrier = IVersionedCarbonCarrier.class.cast(unit);
							db.getDB().exec(String.format("INSERT INTO VersionedCarbonCarrier (owner, name, turn, type, orderOwner, orderSource, orderPriority) VALUES (%s, %s, %s, %s, %s, %s, %s);", "'"+versionedCarbonCarrier.getOwner()+"'", "'"+versionedCarbonCarrier.getName()+"'", "'"+versionedCarbonCarrier.getTurn()+"'", "'"+versionedCarbonCarrier.getType()+"'", "'"+versionedCarbonCarrier.getOrderOwner()+"'", "'"+versionedCarbonCarrier.getOrderSource()+"'", "'"+versionedCarbonCarrier.getOrderPriority()+"'").replaceAll("'null'", "NULL"));
						}
						break;
					}
					case SpaceRoadDeliverer:
					{
						...
						{
							IVersionedSpaceRoadDeliverer versionedSpaceRoadDeliverer = IVersionedSpaceRoadDeliverer.class.cast(unit);
							db.getDB().exec(String.format("INSERT INTO VersionedSpaceRoadDeliverer (owner, name, turn, type) VALUES (%s, %s, %s, %s);", "'"+versionedSpaceRoadDeliverer.getOwner()+"'", "'"+versionedSpaceRoadDeliverer.getName()+"'", "'"+versionedSpaceRoadDeliverer.getTurn()+"'", "'"+versionedSpaceRoadDeliverer.getType()+"'").replaceAll("'null'", "NULL"));
						}
						break;
					}
				}
